...
export const AccordionItem = forwardRef<HTMLDivElement, IAccordionItemProps>(
	({ id, icon, title, htmlTitle, children, tag, headerTag, overWriteColor, classNameHeader, classNameBody, ...props }, ref) => {
		...
		return (
			<TagWrapper tag={tag} ref={ref} className={classNames('accordion-item')}>
				...
				<Collapse
					...
					<div className={classNames('accordion-body', classNameBody)}>{children}</div>
				</Collapse>
			</TagWrapper>
		);
	},
