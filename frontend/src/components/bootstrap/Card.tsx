...
export const CardTabItem: FC<ICardTabItemProps> = ({ id, title, icon, children, hidden }) => {
	throw new Error(
...
const Card = forwardRef<HTMLDivElement, ICardProps>(
	(
		...
		// Handle action when activeTab is triggered
		const _handleActiveTab = (index: number) => {
			setActiveTab(index);
			if (onActiveTab !== undefined) onActiveTab(index);
		}
...
/* 						[`card-stretch-${stretch === 'semi' ? 'semi' : 'full'}`]: stretch,
				...
				{!hasTab ? (
					...
					<>
						<CardHeader borderSize={1} className='justify-content-start'>
							<br />
							<CardActions>
								{Children.toArray(children).filter((child: any) => {
									return child.props.hidden === undefined || child.props.hidden === false
								}).map((item, index) => (
									<Button
										// @ts-ignore
										key={item.props.id}
										...
										onClick={() => _handleActiveTab(index)}>
										{/* @ts-ignore */}
										{item.props.title}
									</Button>
								))}
