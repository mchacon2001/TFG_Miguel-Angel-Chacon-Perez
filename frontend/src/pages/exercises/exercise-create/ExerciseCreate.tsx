...
const ExerciseCreate: FC = () => {
	...
	const handleCreation = async (values: any) => {
		try {
			setLoading(true);
			let response = await (await (new ExerciseService()).createExercise(values)).getResponseData();
			if (response.success) {
				toast.success(response.message);
				navigate(exerciseMenu.exercises.path, { replace: true })
			} else {
			...
			<Page container='fluid'>
				<Card className="col-md-9 m-auto">
					<CardHeader borderSize={1} className="d-flex justify-content-start">
						...
						<CardTitle className="fs-3 mb-0 ms-3">Crear Ejercicio</CardTitle>
					</CardHeader>
					<CardBody>
						<ExerciseForm submit={handleCreation} isLoading={loading} />
					</CardBody>
				</Card>
			</Page>
		</Fragment>
