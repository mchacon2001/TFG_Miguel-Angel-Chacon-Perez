...
interface IExerciseCategoryForm {
...
const ExerciseCategoryForm: FC<ExerciseCategoryFormProps> = ({ isLoading, submit, data }) => {

    const { userCan } = useContext(PrivilegeContext);
    const mode = data ? 'Editar' : 'Crear';

    const initialValues: IExerciseCategoryForm = {
    ...
    const verifyClass = (inputFieldID: keyof IExerciseCategoryForm) => { return (formik.touched[inputFieldID] && formik.errors[inputFieldID]) ? 'is-invalid' : '' };

    const showErrors = (inputFieldID: keyof IExerciseCategoryForm) => {
